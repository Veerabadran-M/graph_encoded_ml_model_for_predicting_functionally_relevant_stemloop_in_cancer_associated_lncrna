...
def secondary_structure_prediction(base_dir, excel, output):

    base = Path(base_dir)
    excel_file = base / "processed-data" / excel

    lncrna_df = pd.read_excel(excel_file)
    ...
    lncrna_df.to_excel(output, index=False)