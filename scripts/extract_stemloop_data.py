...
input_path = "lncrna structure excel path"
folder_path = "folder path to store structural analysis xml files"

reference_excel = pd.read_excel(input_path)

reference_excel = reference_excel.dropna(subset=['Secondary_structure_representation']).reset_index(drop=True)
...
XML_FOLDER = "XML folder path"

def extract_stemloop_data(xml_file):
...
for xml_file in os.listdir(XML_FOLDER):
    ...
    stemloop_df = pd.concat([stemloop_df, extract_stemloop_data(os.path.join(XML_FOLDER, xml_file))], ignore_index=True)
    count += 1

print_loading_bar(total, total, 50, f"All files parsed.")

stemloop_df.to_excel("desired output file path", index=False)